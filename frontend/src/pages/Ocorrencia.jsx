      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div className="flex bg-gray-50 rounded-lg p-1">
            <button className="p-2 hover:bg-white hover:shadow-sm rounded-md transition">
              <ChevronLeft size={20} />
            </button>
            <button className="p-2 hover:bg-white hover:shadow-sm rounded-md transition">
          ...
          <button className="text-sm font-medium text-blue-600 px-3">
            Hoje
          </button>
        </div>
        ...
        <div className="grid grid-cols-6 border-b border-gray-100 bg-gray-50/50">
          ...
          {diasSemana.map((d, i) => (
            ...
            >
              ...
              <p
                className={`text-xl font-bold ${d.dia === "25" ? "text-blue-600" : "text-gray-700"}`}
              >
                {d.dia}
              </p>
            </div>
        ...
        <div className="grid grid-cols-6 relative h-[600px] overflow-y-auto">
          ...
          <div className="col-span-1 border-r border-gray-100 bg-gray-50/30">
            {horarios.map((hora) => (
              <div
                key={hora}
                className="h-20 border-b border-gray-100 p-2 text-right text-xs font-medium text-gray-400"
              >
                {hora}
              </div>
            ))}
          ...
          {[1, 2, 3, 4, 5].map((dia) => (
            <div
              key={dia}
              className="col-span-1 border-r border-gray-100 last:border-0 relative"
            >
              {horarios.map((hora) => (
                <div
                  key={hora}
                  className="h-20 border-b border-gray-100 hover:bg-blue-50/30 transition cursor-pointer"
                ></div>
              ))}
              ...
              {dia === 5 && (
                <AppointmentCard
                  top="top-[320px]"
                  height="h-20"
                  name="Homem com Objetos Suspeitos"
                  type="Resolvido"
